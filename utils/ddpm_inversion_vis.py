...
def inversion_forward_process(model, x0,
    ...
    if etas is None or (type(etas) in [int, float] and etas == 0):
        ...
        xts = ddpm_sample_xts_from_x0(model, x0, num_inference_steps=num_inference_steps)
        # xts = ddpm_sample_xts_from_xts1(model, x0, num_inference_steps=num_inference_steps)
        alpha_bar = model.scheduler.alphas_cumprod
        zs = torch.zeros(size=variance_noise_shape, device=model.device)
        directions = torch.zeros(size=variance_noise_shape, device=model.device)
    ...
    for t in op:
        ...
        if eta_is_zero:
            ...
            directions[idx] = pred_sample_direction

            mu_xt = alpha_prod_t_prev ** (0.5) * pred_original_sample + pred_sample_direction

            # directions[idx] = mu_xt
...
def ddpm_sample_xts_from_x0(model, x0, num_inference_steps=50):
    ...
    for t in reversed(timesteps):
        idx = t_to_idx[int(t)]
        noise = torch.randn(x0.shape, device=x0.device)
        xts[idx] = add_noise(model.scheduler, x0, noise, t)
    xts = torch.cat([xts, x0], dim=0)
